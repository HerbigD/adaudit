...
async def human_review(state: AuditState) -> dict:
    ...
    with events.step("human_review") as t:
        ...
        if choice == "prediction" and revised is not None:
            ...
            final = (
                ...
                if initial
                    ...
                    general_category=taxonomy.GENERAL_CATEGORIES[8],
                    specific_code=33,
        ...
        t.summary = f"人工采纳 {choice} → [{final.specific_code}]"
        return {
            "human_choice": choice,
...
def _reason(state: AuditState) -> str:
    ...
    initial = state.get("initial")
    if initial and not initial.name_or_brand_legible:
        return "无法识别产品名/品牌"
    return "置信度不足"
